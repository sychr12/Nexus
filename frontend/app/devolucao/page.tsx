...
import { AlertTriangle, ChevronDown, Eye, FileText, Paperclip, RotateCcw, Search, X } from "lucide-react";
...
const normalizarSituacao = (consulta: string) => {
  ...
  return { label: texto || "Não informado", className: "bg-amber-50 text-amber-800 ring-amber-100" };
};
...
const getDetalhesProcesso = (item: EncaminhamentoAnalise) =>
  [
    ...
    ["Motivo técnico", item.resultadoConsulta],
    ...
    ["Observação", item.observacao || "Sem observação registrada na análise."],
                    ...
                    <div key={item.id} className="rounded-md border p-3" style={{ borderColor: COLORS.border, backgroundColor: COLORS.card }}>
                      ...
                      <div className="mt-3 grid grid-cols-[1fr_auto] items-end gap-3">
                        ...
                        <button
                          type="button"
                          disabled={!isAdmin}
                          title={isAdmin ? "Devolver processo" : "Apenas administrador pode alterar esta etapa"}
                          ...
                          style={{ backgroundColor: isAdmin ? COLORS.danger : COLORS.textLight }}
                        >
                          <RotateCcw size={14} />
                          {isAdmin ? "Devolver" : "Bloqueado"}
                        </button>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <button
                          type="button"
                          onClick={() => setExpandedItemId(isExpanded ? null : item.id)}
                          className="inline-flex items-center gap-2 rounded-md px-2.5 py-1.5 text-xs font-semibold transition-colors hover:bg-[#FEF3F2]"
                        ...
                        <button
                          type="button"
                          onClick={() => setExpandedItemId(isExpanded ? null : item.id)}
                          className="inline-flex items-center gap-1 rounded-md px-2.5 py-1.5 text-xs font-semibold transition-colors hover:bg-[#FEF3F2]"
                            ...
                            <div className="mt-1 space-y-1">
                              {arquivos.map((arquivo) => (
                                <button
                                  ...
                                  className="flex w-full min-w-0 items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs transition-colors hover:bg-[#FEF3F2]"
                                  style={{ color: COLORS.textLight }}
                                >
                                  <Eye size={14} className="shrink-0" style={{ color: COLORS.danger }} />
                    ...
                    {filteredItems.map((item) => {
                      ...
                      return (
                        <Fragment key={item.id}>
                          <tr className="border-t align-middle transition-colors hover:bg-[#FFF8F7]" style={{ borderTopColor: COLORS.border }}>
                                    ...
                                    <div className="mt-2 space-y-1">
                                      {arquivos.map((arquivo) => (
                                        <button
                                          ...
                                          onClick={() => openViewer(item, arquivo.kind)}
                                          title={`Visualizar ${arquivo.file}`}
                                          className="group flex w-full min-w-0 items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs transition-all duration-200 hover:bg-[#FEF3F2]"
                                          style={{ color: COLORS.textLight }}
                                        >
                                          <Eye size={14} className="shrink-0" style={{ color: COLORS.danger }} />
                                          <span className="font-semibold" style={{ color: COLORS.text }}>{arquivo.label}:</span>
                                          <span className="truncate transition-colors group-hover:text-[#B42318]">{arquivo.file}</span>
                                        </button>
                                      ))}
                                    </div>
                                  </div>
